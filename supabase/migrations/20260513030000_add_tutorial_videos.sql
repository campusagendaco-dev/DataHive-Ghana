-- MIGRATION: ADD TUTORIAL VIDEO COLUMNS
-- Extends the system settings to support 3 customizable tutorial video links for each customer path
-- Rebuilds public_system_settings to grant frontend clients secure lookup rights.

ALTER TABLE public.system_settings 
ADD COLUMN IF NOT EXISTS tutorial_buy_video_url TEXT,
ADD COLUMN IF NOT EXISTS tutorial_agent_video_url TEXT,
ADD COLUMN IF NOT EXISTS tutorial_subagent_video_url TEXT;
...
-- Grant access to public queries
GRANT SELECT ON public.public_system_settings TO anon, authenticated, service_role;

COMMENT ON VIEW public.public_system_settings IS 'Fully unified public system settings view including video tutorial URLs.';
