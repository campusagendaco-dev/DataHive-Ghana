-- Manual approval for willlywilly8000@gmail.com
DO $$
DECLARE
    target_user_id UUID;
BEGIN
    -- Try to find in profiles first (case insensitive)
    SELECT user_id INTO target_user_id FROM public.profiles WHERE email ILIKE '%willlywilly8000%' LIMIT 1;
    ...
    ELSE
        RAISE EXCEPTION 'User matching willlywilly8000 not found in auth.users or profiles';
    END IF;
END $$;
