-- Wallet Deduction for Duplicated Restoration
-- This script reverses the hardcoded amounts in admin_apply_wallet_restoration
-- to fix accounts that were credited multiple times.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.audit_logs WHERE action = 'wallet_deduction_applied' LIMIT 1
  ) THEN
    RAISE NOTICE 'Deduction already applied. Skipping.';
    RETURN;
  END IF;
  ...
  UPDATE public.wallets SET balance = balance - 20.00    WHERE agent_id = 'b690b375-1366-400c-a192-7092ab1f1435'; _restored := _restored + 1;
  ...
  VALUES (
    'wallet_deduction_applied',
    jsonb_build_object(
      'message', format('Wallet deduction complete. Adjusted balances for %s agents.', _restored),
