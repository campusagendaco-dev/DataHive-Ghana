-- Fix: api.create_order_rpc inserts orders directly as 'fulfilled' (no UPDATE trigger fires),
-- so credit_order_profits was never called → parent agents of sub-agent API users
-- never received their referral commission. Added explicit call after INSERT.
...
BEGIN
  ...
  IF v_parent_profit > 0 AND v_parent_agent_id IS NOT NULL THEN
