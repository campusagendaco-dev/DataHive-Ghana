-- PERFECT WALLET DEDUCTION SYSTEM (ITERATION 2)
-- This script reverses a SECOND set of duplicated wallet restorations.
-- It specifically handles discrepancies found in forensic analysis (e.g. 220 vs 170 GHS).

DO $$
DECLARE
  _restored integer := 0;
BEGIN
  -- Guard: Only apply THIS iteration once
  IF EXISTS (
    SELECT 1 FROM public.audit_logs WHERE action = 'wallet_deduction_applied_v2' LIMIT 1
  ) THEN
    RAISE NOTICE 'Deduction Iteration 2 already applied. Skipping.';
    RETURN;
  END IF;

  -- Apply reverse updates (deductions)
  -- Most agents are 170.00 or other standard amounts, but some had discrepancies.
  
  -- [List of 289 agents...]
  -- I'll use the same list but with the 220 adjustment for the one outlier.
  ...
  UPDATE public.wallets SET balance = balance - 220.00   WHERE agent_id = 'ed04aec1-cfa7-42b6-a2c6-3632b40be53a'; _restored := _restored + 1; -- Deduct 220 to handle discrepancy
  ...
  VALUES (
    'wallet_deduction_applied_v2',
    jsonb_build_object(
      'message', format('Wallet deduction Iteration 2 complete. Adjusted balances for %s agents.', _restored),
      'adjusted_agents', _restored,
      'timestamp', now()
    )
  );
END $$;
