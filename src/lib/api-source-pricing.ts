...
export async function fetchApiPricingContext(): Promise<{ source: ApiSource; multiplier: number }> {
  const { data } = await supabase.functions.invoke("system-settings", { body: { action: "get" } });
  const source: ApiSource = data?.active_api_source === "secondary" ? "secondary" : "primary";
  const customMarkupPct = Number(data?.secondary_price_markup_pct);
