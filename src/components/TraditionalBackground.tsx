...
export const TraditionalBackground = ({ className = "fixed inset-0 z-0" }: { className?: string }) => {
  ...
  // Combined styles with dynamic className
  const baseClasses = "w-full h-full overflow-hidden pointer-events-none select-none";

  // CASE A: Custom Image Background provided by Admin overrides symbols!
  if (customBgUrl) {
    return (
      <div 
        className={`${className} ${baseClasses} bg-cover bg-center bg-no-repeat transition-all duration-1000`}
        style={{ 
          backgroundImage: `url(${customBgUrl})`,
          opacity: 0.3 // Boosted significantly for higher dynamic range on phone screens
        }} 
      />
    );
  }
  ...
  return (
    <div className={`${className} ${baseClasses} opacity-[0.15] dark:opacity-[0.25]`}>
      <style>
        {`
