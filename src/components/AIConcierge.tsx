...
export default function AIConcierge() {
  ...
  const send = async (text: string) => {
    ...
    try {
      if (user) {
        supabase.from("chat_messages").insert({ user_id: user.id, role: "user", content: msg })
          .then(({ error }) => { if (error) console.warn("[AIConcierge] Failed to save user message:", error); });
      }
      ...
      if (user) {
        supabase.from("chat_messages").insert({ user_id: user.id, role: "bot", content: reply })
          .then(({ error }) => { if (error) console.warn("[AIConcierge] Failed to save bot reply:", error); });
      }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-1.5">
            ...
            <motion.span animate={{ rotate: [0, 20, -20, 0] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }} className="text-xs">✨</motion.span>
          </div>
          <div className="flex items-center gap-1.5">
      ...
      {messages.map((m, i) => (
        ...
        >
          ...
          <div className={cn("flex flex-col gap-1 max-w-[78%]", m.role === "user" ? "items-end" : "items-start")}>
            <div
              className={cn(
                "px-4 py-2.5 rounded-2xl text-sm leading-relaxed font-medium",
                m.role === "user" ? "text-white rounded-tr-sm" : "text-slate-200 rounded-tl-sm"
              )}
              style={m.role === "user"
      ...
      <AnimatePresence>
        {typing && (
          ...
          >
            ...
            <div className="px-4 py-3 rounded-2xl rounded-tl-sm flex gap-1.5 items-center" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
              {[0, 0.18, 0.36].map((delay, i) => (
                <motion.span key={i} animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay, ease: "easeInOut" }} className="w-1.5 h-1.5 rounded-full bg-indigo-400" />
              ))}
            </div>
          </motion.div>
        )}
  ...
  const QuickActions = () => (
    <div className="px-4 py-2 flex gap-2 overflow-x-auto scrollbar-none shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
      {QUICK_ACTIONS.map(q => (
        <button
          key={q}
          type="button"
          onClick={() => send(q)}
          disabled={typing}
          className="shrink-0 px-3 py-2 rounded-xl text-[11px] font-bold whitespace-nowrap transition-all text-indigo-300 hover:text-white disabled:opacity-40 active:scale-95"
  ...
  const InputBar = () => (
    <div className="px-4 pb-[env(safe-area-inset-bottom,16px)] pt-3 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      <div className="flex items-center gap-2">
        <input
          ref={inputRef}
          value={input}
  ...
  const panelBg = {
    background: "linear-gradient(160deg,rgba(15,15,26,0.97) 0%,rgba(10,10,18,0.99) 60%,#080810 100%)",
          ...
          {open && (
            <>
              ...
              <motion.div
                ...
                style={{ height: "92dvh", ...panelBg, boxShadow: "0 -24px 60px rgba(0,0,0,0.7)" }}
              >
                {/* Drag handle */}
          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                style={{ height: "min(560px, 85dvh)", ...panelBg, boxShadow: "0 32px 80px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.04) inset" }}
              >
